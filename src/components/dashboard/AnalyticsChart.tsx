...
export const AnalyticsChart: React.FC<AnalyticsChartProps> = ({ 
  ...
  const getChartData = () => {
    ...
    if (currentUser?.role === 'dxm' || currentUser?.role === 'xerenteATSXPTPG' || currentUser?.role === 'admin') {
    ...
    <Card className="col-span-1">
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            {currentUser?.role === 'dxm' || currentUser?.role === 'xerenteATSXPTPG' || currentUser?.role === 'admin'
              ? 'Estado das tarefas' 
              ...
              {getChartData().length > 0 ? (
                <BarChart data={getChartData()}>
                  ...
                  <Bar 
                    ...
                    name={currentUser?.role === 'dxm' || currentUser?.role === 'xerenteATSXPTPG' || currentUser?.role === 'admin' ? 'Tarefas' : 'Horas'} 
                  />
                </BarChart>
              ) : (
